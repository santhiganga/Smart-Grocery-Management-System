...
class DemandForecaster:
    ...
    def forecast(self, 
        ...
        try:
            ...
            result = {
                ...
                'components': forecast[['ds', 'trend', 'yearly', 'weekly']] if return_components else None
            }
    ...
    def get_seasonal_patterns(self) -> Dict:
        ...
        if self.model.yearly_seasonality:
            ...
            start_date = pd.Timestamp('2020-01-01')  # arbitrary start
            yearly_dates = pd.date_range(start=start_date, periods=365, freq='D')
            yearly_df = pd.DataFrame({'ds': yearly_dates})
            yearly_forecast = self.model.predict(yearly_df)
            patterns['yearly'] = yearly_forecast[['ds', 'yearly']]
            
        # Extract weekly seasonality
        if self.model.weekly_seasonality:
            # Create one week of dates
            start_date = pd.Timestamp('2020-01-01')  # arbitrary start
            weekly_dates = pd.date_range(start=start_date, periods=7, freq='D')
            weekly_df = pd.DataFrame({'ds': weekly_dates})
            weekly_forecast = self.model.predict(weekly_df)
            patterns['weekly'] = weekly_forecast[['ds', 'weekly']]
